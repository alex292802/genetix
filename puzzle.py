...
class puzzle:
    ...
    def __init__(self, size=int, pieces=np.ndarray):
        ...
        self.inn_bords = [i for i in range(1, self.s - 1)] + [i * self.s + self.s - 1 for i in range(1, self.s - 1)] + \
                        [i for i in range(self.s * (self.s - 1) + 1, self.s ** 2 - 1)] + [i * self.s for i in range(1, self.s - 1)]
        self.inn_milieu = list(np.array([[i + self.s * j for i in range(1, self.s - 1)] for j in range(1, self.s - 1)]).flatten())
    ...
    def mutation_swap(self):
        ...
        if self.conf[a].cat == 2:
            ...
            self.conf[a].x, self.conf[b].x, self.conf[a].y, self.conf[b].y = self.conf[b].x,self.conf[a].x, self.conf[b].y,self.conf[a].y
